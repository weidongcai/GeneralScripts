...
"""
Adapted from matlab getFiltCoeffs which is originally from eegfilt?
"""
def get_filt_coeffs(data,srate,locutoff=0,hicutoff=0,epochframes=0,filtorder=0,revfilt=False):
    ...
    if filtorder == 0:
        if locutoff > 0: 
            filtorder = minfac * np.fix(srate/locutoff)
        elif hicutoff > 0:
            filtorder = minfac * np.fix(srate/hicutoff)
    ...
    epochs = np.fix(frames/epochframes)

    assert epochs*epochframes == frames, 'error: epochframes does not divide frames'
    assert filtorder*3 <= epochframes, 'error: epochframes must be at least 3 times the filtorder'
    assert (1+trans)*hicutoff/nyq <= 1, 'error: high cutoff freq too close to Nyquist frequency'

    if (locutoff > 0) and (hicutoff > 0): #bandpass
        f=[MINFREQ, (1.-trans)*locutoff/nyq, locutoff/nyq, hicutoff/nyq, (1.+trans)*hicutoff/nyq, 1.]
        m=[0., 0., 1., 1., 0., 0.]
    elif locutoff > 0: #highpass filter
        assert locutoff/nyq >= MINFREQ, 'highpass cutoff freq must be > %f Hz' % MINFREQ*nyq
        f=[MINFREQ, locutoff*(1.-trans)/nyq, locutoff/nyq, 1.]
        m=[0., 0., 1., 1.]
    elif hicutoff > 0: #                %  lowpass filter
        assert hicutoff/nyq >= MINFREQ, 'lowpass cutoff freq must be > %f Hz' % MINFREQ*nyq
        f=[MINFREQ,hicutoff/nyq,hicutoff*(1+trans)/nyq,1.]
        m=[1., 1., 0., 0.]
    else:
        'one of lowcutoff or highcutoff must be set!'

    if revfilt:
        m = not m

    return firls(filtorder, f, m) # get FIR filter coefficients
