function mars_rois2img_wLabels(roi_list, img_name, roiIntensityFname, roi_space, flags)
...
% read ROI intensity
intensities = csvread(roiIntensityFname);
roi_intensity = intensities(:,2);
...
% 				   'number', roi_ctr);
    img_data(dat ~= 0) = roi_intensity(i);
    roi_info(roi_ctr) = struct('label', label(roi),...
        'number', roi_intensity(i));
      else
